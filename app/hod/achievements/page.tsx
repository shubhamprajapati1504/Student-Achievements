...
export default function HODAchievementsPage() {
  ...
  const fetchAchievements = async () => {
    try {
      ...
      const response = await fetch(`/api/hod/achievements?${params.toString()}`)
      if (response.ok) {
  ...
  return (
    <div>
      <Navbar />
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Department Achievements</h1>
          ...
          <ul className="divide-y divide-gray-200">
            {achievements.map((achievement) => (
              <li key={achievement.id}>
                <Link href={`/hod/achievements/${achievement.id}`} className="block hover:bg-gray-50">
